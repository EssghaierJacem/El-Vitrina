  ...
  private loadStore(): void {
    ...
    this.storeService.getById(this.storeId).subscribe({
      next: (store) => {
        ...
        if (store.image) {
          this.imagePreviewUrl = `http://localhost:8080/api/stores/store/images/${store.image}`;
        }
        if (store.coverImage) {
          this.coverImagePreviewUrl = `http://localhost:8080/api/stores/store/images/${store.coverImage}`;
        }
